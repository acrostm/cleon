...
const getErrorMessage = (error: unknown) =>
  error instanceof Error ? error.message : "Failed to request Xiaohongshu verification code";

const normalizePhoneNumber = (value: unknown) =>
  typeof value === "string" ? value.replace(/\D/g, "").slice(0, 15) : "";

const maskPhoneNumber = (phoneNumber: string) =>
  phoneNumber.length > 7 ? `${phoneNumber.slice(0, 3)}****${phoneNumber.slice(-4)}` : "masked";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const response = await fetch(`${workerBaseUrl}/auth/request-code`, {
      method: "POST",
      headers: {
    ...
    await recordArchiveAudit({
      action: "AUTH_PROFILE_VERIFICATION_CODE_REQUESTED",
    ...
    console.error("[Archive Auth Profile Request Code Error]:", error);
